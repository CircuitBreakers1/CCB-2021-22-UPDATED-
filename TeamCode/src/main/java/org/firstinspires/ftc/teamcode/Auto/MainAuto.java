...
import static org.firstinspires.ftc.teamcode.Subsystems.Robot.blinkinLedDriver;
...
import static org.firstinspires.ftc.teamcode.Subsystems.Robot.pattern;
...
public class MainAuto extends AutoSwitcher {
    ...
    public void initAuto() {
        this.robot.init(opMode.hardwareMap, 35.8, 7, 90, NO_CAM);
        setMirror(35.8, 7, 90);
        ...
        this.opMode.telemetry.addData("Color Guess", colorSleevePipeline.getPredictedColor().toString());
        this.opMode.telemetry.addData("Hue Average", colorSleevePipeline.getAverage());
        this.opMode.telemetry.update();

        this.color = colorSleevePipeline.getPredictedColor();
    ...
    public void runAuto() {
        ...
        while(this.opMode.opModeIsActive()) {
            ...
            if(getRemainingTime(startTime, this.opMode.getRuntime()) < (pickupTime + parkFromLine[(int) mirror(color.getValue(), 1)])) {
                break;
            }

            //Pickup the cone
            moveToMirrored(11.3, 59.75, 0.7);
            pickupLeft.setPower(1);
            pickupRight.setPower(-1);
...
//            LiftSubsystem.setTarget(Min);
            ...
            if(getRemainingTime(startTime, this.opMode.getRuntime()) < dropTime + movementTime + parkFromJunction[(int) mirror(color.getValue(), 1)]) {
                break;
            }

            //Drop off the cone
            moveToMirrored(48.5, 58.25, 0.7);
            turnMirrored(90, 0.7);
            moveToMirrored(49, 61.25, 0.7);
            pickupLeft.setPower(-1);
            pickupRight.setPower(1);
            ...
            if(getRemainingTime(startTime, this.opMode.getRuntime()) < movementTime + pickupTime + parkFromLine[(int) mirror(color.getValue(), 1)]) {
                break;
            }
